...
class Finger(object):
    ...
    def __init__(self, uid, fid, valid, template):
        ...
        self.mark = codecs.encode(
            template[:8], 'hex') + b'...' + codecs.encode(template[-8:], 'hex')
    ...
    def dump(self):
        return "<Finger> [uid:{:>3}, fid:{}, size:{:>4} v:{} t:{}]".format(
            self.uid, self.fid, self.size, self.valid,
...
class User(object):
    ...
    def repack29(self):  # with 02 for zk6 (size 29)
        return pack("<BHB5s8sIxBhI", 2, self.uid, self.privilege,
                    self.password.encode(
                        User.encoding, errors='ignore'), self.name.encode(
                        User.encoding, errors='ignore'), self.card, int(
                        self.group_id) if self.group_id else 0, 0, int(
                        self.user_id))
    ...
    def repack73(self):  # with 02 for zk8 (size73)
        ...
        return pack("<BHB8s24sIB7sx24s", 2, self.uid, self.privilege,
                    self.password.encode(
                        User.encoding, errors='ignore'), self.name.encode(
                        User.encoding, errors='ignore'), self.card, 1, str(
                        self.group_id).encode(
                        User.encoding, errors='ignore'), str(
                        self.user_id).encode(
                        User.encoding, errors='ignore'))

    def __str__(self):
        return '<User>: [uid:{}, name:{} user_id:{}]'.format(
...
class ZK_helper(object):
    ...
    def test_ping(self):
        ...
        ping_str = "-n 1" if platform.system().lower() == "windows" \
            else "-c 1 -W 5"
...
class ZK(object):
    ...
    def __create_tcp_top(self, packet):
        ...
        top = pack('<HHI', const.MACHINE_PREPARE_DATA_1,
                   const.MACHINE_PREPARE_DATA_2, length)
        return top + packet
    ...
    def __test_tcp_top(self, packet):
        ...
        if tcp_header[0] == const.MACHINE_PREPARE_DATA_1 and \
                tcp_header[1] == const.MACHINE_PREPARE_DATA_2:
            return tcp_header[2]
    ...
    def __send_command(self, command, command_string=b'', response_size=8):
        ...
        '''
        buf = self.__create_header(
            command, command_string, self.__session_id, self.__reply_id)
        try:
            if self.tcp:
    ...
    def __ack_ok(self):
        """ event ack ok """
        buf = self.__create_header(
            const.CMD_ACK_OK, b'', self.__session_id, const.USHRT_MAX - 1)
        try:
            if self.tcp:
                top = self.__create_tcp_top(buf)
                self.__sock.send(top)
    ...
    def connect(self):
        ...
        if not self.ommit_ping and not self.helper.test_ping():
            raise ZKNetworkError(
                "can't reach device (ping %s)" % self.__address[0])
    ...
    def get_serialnumber(self):
        ...
        cmd_response = self.__send_command(
            command, command_string, response_size)
        if cmd_response.get('status'):
    ...
    def _clear_error(self, command_string=b''):
        """ clear ACK_error """
        cmd_response = self.__send_command(
            const.CMD_ACK_ERROR, command_string, 1024)
        # cmd_response['code'] shuld be CMD_ACK_UNKNOWN
        cmd_response = self.__send_command(
            const.CMD_ACK_UNKNOWN, command_string, 1024)
        cmd_response = self.__send_command(
            const.CMD_ACK_UNKNOWN, command_string, 1024)
        cmd_response = self.__send_command(
            const.CMD_ACK_UNKNOWN, command_string, 1024)
    ...
    def get_network_params(self):
        ...
        gate = b''
        cmd_response = self.__send_command(
            const.CMD_OPTIONS_RRQ, b'IPAddress\x00', 1024)
        if cmd_response.get('status'):
            ip = (self.__data.split(b'=', 1)[-1].split(b'\x00')[0])
        cmd_response = self.__send_command(
            const.CMD_OPTIONS_RRQ, b'NetMask\x00', 1024)
        if cmd_response.get('status'):
            mask = (self.__data.split(b'=', 1)[-1].split(b'\x00')[0])
        cmd_response = self.__send_command(
            const.CMD_OPTIONS_RRQ, b'GATEIPAddress\x00', 1024)
        if cmd_response.get('status'):
    ...
    def read_sizes(self):
        ...
        if cmd_response.get('status'):
            ...
            if len(self.__data) >= 12:  # face info
                # dirty hack! we need more information
    ...
    def __str__(self):
        """ for debug"""
        return "ZK %s://%s:%s users[%i]:%i/%i fingers:%i/%i,\
             records:%i/%i faces:%i/%i" % (
    ...
    def get_users(self):  # ALWAYS CALL TO GET correct user_packet_size
        ...
        users = []
        max_uid = 0
        userdata, size = self.read_with_buffer(
        ...
        if self.user_packet_size == 28:
            while len(userdata) >= 28:
                ...
                if not name:
                    name = "NN-%s" % user_id
                user = User(uid, name, privilege, password,
                            group_id, user_id, card)
                users.append(user)
                if self.verbose:
                    print("[6]user:", uid, privilege, password,
                          name, card, group_id, timezone, user_id)
            ...
            while len(userdata) >= 72:
                ...
                name = (name.split(b'\x00')[0]).decode(
                    self.encoding, errors='ignore').strip()
                group_id = (group_id.split(b'\x00')[0]).decode(
                    self.encoding, errors='ignore').strip()
                user_id = (user_id.split(b'\x00')[0]).decode(
                    self.encoding, errors='ignore')
                if uid > max_uid:
                    max_uid = uid
    ...
    def __recieve_tcp_data(self, data_recv, size):
        ...
        if (tcp_length - 8) < size:  # broken on smaller DATAs
            ...
            if self.verbose:
                print(
                    "new tcp DATA starting with {} bytes".format(
                        len(data_recv)))
            resp, bh = self.__recieve_tcp_data(data_recv, size)
            data.append(resp)
            if self.verbose:
                print("for misssing {} recieved {} with extra {}".format(
                    size, len(resp), len(bh)))
        ...
        if recieved >= (size + 32):  # complete with ACK_OK included
            ...
            if self.verbose:
                print(
                    "try DATA incomplete (actual valid {})".format(
                        recieved - 16))
    ...
    def __recieve_chunk(self):
        ...
        if self.__response == const.CMD_DATA:  # less than 1024!!!
            if self.tcp:  # MUST CHECK TCP SIZE
                if self.verbose:
                    print("_rc_DATA! is {} bytes, tcp length is {}".format(
                        len(self.__data), self.__tcp_length))
            ...
            if self.tcp:
                if self.verbose:
                    print("recieve chunk: len data is {}".format(len(self.__data)))
                # ideally 8 bytes of PREPARE_DATA only...
                # but sometimes it comes with data...

                # prepare data with actual data! should be 8+size+32
                if len(self.__data) >= (8 + size):
                    # no need for more data! test, maybe -32
                    data_recv = self.__data[8:]
                else:
                    # could have two commands
                ...
                # could be broken
                if len(data_recv) < 16:
                    print("trying to complete broken ACK %s /16" %
                          len(data_recv))
                    if self.verbose:
    ...
    def read_with_buffer(self, command, fct=0, ext=0):
        ...
        if cmd_response['code'] == const.CMD_DATA:
            ...
            if self.tcp:  # MUST CHECK TCP SIZE
                if self.verbose:
                    print("DATA! is {} bytes, tcp length is {}".format(
                        len(self.__data), self.__tcp_length))
                if len(self.__data) < (self.__tcp_length - 8):
                    need = (self.__tcp_length - 8) - len(self.__data)
                    if self.verbose:
                        print("need more data: {}".format(need))
                    more_data = self.__recieve_raw_data(need)
                    return b''.join([self.__data, more_data]), len(
                        self.__data) + len(more_data)
                else:  # enough data
                    if self.verbose:
                        print("Enough data")
                    size = len(self.__data)
        ...
        if self.verbose:
            print(
                "rwb: #{} packets of max {} bytes, and extra {} bytes remain"
                .format(packets, MAX_CHUNK, remain))
        for _wlk in range(packets):
    ...
    def get_attendance(self):
        ...
        if record_size == 8:  # ultra old format
            while len(attendance_data) >= 8:  # TODO RETEST ZK6!!!
                uid, status, timestamp, punch = unpack(
                    'HB4sB', attendance_data.ljust(8, b'\x00')[:8])
                if self.verbose:
                ...
                attendance = Attendance(
                    user_id, timestamp, status, punch, uid)  # punch?
            ...
            while len(attendance_data) >= 16:  # TODO RETEST ZK6
                ...
                if not tuser:
                    ...
                    uid = str(user_id)
                    tuser = list(filter(lambda x: x.uid ==
                                        user_id, users))  # refix
                    if not tuser:
