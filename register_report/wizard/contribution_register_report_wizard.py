...
#     branch_id = fields.Many2many('res.branch',string="Branch")
    ...
    def show_register_summery_report_pdf(self):
        ...
        report_id = self.env['ir.actions.report'].with_context(context).search(
            [('report_name', '=', 'register_report.contribution_register_summery_report_template_id')], limit=1)
        if not report_id:
            raise UserError(
    ...
    def show_register_report_pdf(self):
        context = self.env.context
        report_id = self.env['ir.actions.report'].with_context(context).search(
            [('report_name', '=', 'register_report.contribution_register_report_template_id')], limit=1)
        if not report_id:
            raise UserError(
...
#             }
    ...
    def show_register_report_xlsx(self):
        ...
        context = self.env.context
        report_id = self.env['ir.actions.report'].with_context(context).search(
            [('report_name', '=', 'register_report.register_report_xls')], limit=1)
